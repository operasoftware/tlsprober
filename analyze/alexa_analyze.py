...
"""Report Renego patched/unpatched based on Alexa ranking"""
...
if options.run_id:
	...
	patched = main_result_list.GetAnalyze(
				filter = {Results.ResultSummaryList.QUERY_CONDITION:[Results.ResultCondition.RESULTC_RENEGO]},
				summaries = {"hosts":[Results.ResultSummaryList.RESULT_HOSTS]}
						)
	unpatched_renego = main_result_list.GetAnalyze(
				filter = {Results.ResultSummaryList.QUERY_CONDITION:[Results.ResultCondition.RESULTC_NONRENEGO,Results.ResultCondition.RESULTC_PERFORM_RENEGO]},
				summaries = {"hosts":[Results.ResultSummaryList.RESULT_HOSTS]}
						)
	...
	total_renego = 0
	import csv
	file = csv.writer(open("alexa_renego_rating.csv","wb"))
