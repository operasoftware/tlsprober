...
"""
Prints information about how many servers per hour the scanner have checked
...
while True:
	try:
		...
		def check_performance():
			...
			#if time.clock()- start >= 4*60:
			#	do_analyze_step()
			#	start = time.clock()
					
			print "=======Scanner=============", run.id
			Q = Scanner.ScannerAction.objects.filter(scanner_run=run)
			current_time = datetime.datetime.now()
		
			for node in Scanner.ScannerNode.objects.filter(active_node = True).iterator():
				delta = datetime.timedelta(hours=24)
				Q1 = Q.filter(completed_time__range=(current_time-delta,current_time))
				node_count_24hour = Q1.filter(scanner_node = node).count()

				delta = datetime.timedelta(hours=1)
				Q1 = Q.filter(completed_time__range=(current_time-delta,current_time))
				node_count_hour = Q1.filter(scanner_node = node).count() 
			
				delta = datetime.timedelta(minutes=10)
				Q1 = Q.filter(completed_time__range=(current_time-delta,current_time))
				node_count_10min = Q1.filter(scanner_node = node).count()*6  #recalc to per hour
			
				delta = datetime.timedelta(minutes=1)
				Q1 = Q.filter(completed_time__range=(current_time-delta,current_time))
				node_count_1min = Q1.filter(scanner_node = node).count()*60  #recalc to per hour
			...
			print "All:  %20s: %7d %7d %7d %7d " %(" ",all_count_1min, all_count_10min, all_count_hour, all_count_24hour), " (", Scanner.ScannerQueue.objects.filter(part_of_run=run, state=Scanner.ScannerQueue.SCANQ_IDLE).count(), " left/", Scanner.ScannerResults.objects.filter(part_of_run=run).count(), "found)"
			print "==========================="
		check_performance()
	except:
